...
@Autonomous(name="VTAutoFullRed", group="VT Auto")
public class VTAutoFullRed extends LinearOpMode {
...
/*
        ...
        while (runtime.milliseconds() < 2500){
           // double error = Math.abs(r.servo1.getPosition() - 0.5);
            if (!(r.servo1.getPosition() == 0.5)) {
                double position = r.servo1.getPosition() + (0.0025 * direction);
                position = Range.clip(position, 0, 1);
                r.ArmPosition(position);
...
/*
        ...
        while (runtime.milliseconds() < 2500){
            ...
            r.ArmPosition(0.25); //Arm Down
            if ((int) runtime.milliseconds()%3 == 0) {
                r.ArmPosition(r.servo1.getPosition() - 0.01);
                Thread.sleep(60);
            }
            r.servo3.setPosition(Math.sin(r.ArmAngle.getVoltage()/2.45) + 0.029);
 ...
 */
        ...
        if (stone == 1){
            r.MecanaumStrafe(0.75,85);
        } else if (stone == 2) {
            r.MecanaumStrafe(0.75,-300);
        } else if (stone == 3){
            r.MecanaumStrafe(0.75,-795);
        }
        ...
        r.ArmPosition(0.7); //Arm Up
        r.servo4.setPosition(0.6); //Grabber out
        Thread.sleep(200);

        r.TankForward(0.8, -500);
        r.TankForward(0.8, 100);

        r.IntakePower(-1); //Start collecting
        r.TankForward(0.4, -1200); //Move toward stones
        r.TankForward(0.2, -250); //Move toward stones

        Thread.sleep(750); //Wait for collection
        r.ArmPower(0);

        r.TankForward(0.8, 855); //Move back from stones
        Thread.sleep(100);


        //rotate(90, 0.3); //Rotate to bridge
        //rotateWithArm(90,0.3, -1.0, 7);
        r.servo4.setPosition(0.7);
        /*
        runtime.reset();
        while (runtime.milliseconds() < 2500) {
            r.ArmPower(-0.75); //Arm Down
            r.servo3.setPosition(Math.sin(r.ArmAngle.getVoltage()/2.45) + 0.029);
            if ((int) runtime.milliseconds()%7 == 0) {
                r.ArmPower(0);
                Thread.sleep(3);
            }
        ...
        r.TankForward(0.8, 3550 - (100 * stone)); // Move past bridge
         ...
         */
        r.servo4.setPosition(0.09); //Grab Block


        rotate(90, 0.3); //Rotate to Foundation
        r.TankForward(.25, 545); //Drive to Foundation was 525


        r.foundationMovePower(-1); //Grab Foundation
        Thread.sleep(500);
        r.TankForward(0.8, -1600); //Move forward with foundation was 1500[[
        ...
        TankTurnWithArm(0.3, 2200, 0.15, 3);
        Thread.sleep(200);
    ...
    public void TankTurnWithArm(double power, int target, double ArmPosition, int modulus) throws InterruptedException {
        r.LTMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        r.LBMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        r.RTMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        r.RBMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        if (target != 0) {
            ...
            while (r.LTMotor.isBusy() && r.RTMotor.isBusy()) {
                ...
                r.ArmPosition(ArmPosition); //Arm Down
                if ((int) runtime.milliseconds()%modulus == 0) {
                    r.ArmPosition(r.servo1.getPosition() - 0.01);
                    Thread.sleep(3);
                }
